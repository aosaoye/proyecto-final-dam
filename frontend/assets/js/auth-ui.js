...
export function initGoogleAuth(clientId, containerId = 'google-btn', context = 'login') {
    ...
    if (clientId === 'YOUR_GOOGLE_CLIENT_ID') {
        // Fallback simulated button for local testing without a real client ID
        container.innerHTML = `
            <button type="button" style="width: 100%; display: flex; align-items: center; justify-content: center; gap: 10px; padding: 12px; border: 1px solid #dadce0; border-radius: 4px; background: white; font-family: 'Roboto', sans-serif; font-size: 14px; font-weight: 500; color: #3c4043; cursor: pointer; transition: background 0.2s;">
                ...
                ${context === 'register' ? 'Sign up with Google' : 'Sign in with Google'}
            </button>
        `;
        container.querySelector('button').addEventListener('click', async () => {
            Swal.fire({ title: 'Simulating Google Login...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
            try {
                // Simulate backend registration for the fake user
                await authService.register({ name: 'Google Test User', email: 'test.google@example.com', password: 'oauth-user' }).catch(() => {});
                // Simulate login
                const result = await authService.login('test.google@example.com', 'oauth-user');
                Swal.fire({
                    icon: 'success',
                    title: 'Welcome!',
                ...
                Swal.fire({ icon: 'error', title: 'Simulation Failed', text: err.message });
            }
        });
        return;
    }
